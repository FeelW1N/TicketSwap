...
def event(app):
    with app.app_context():
        e = Event.query.filter_by(
            organizer_id="concert-ru", external_event_id="cr-imagine-2026"
        ).first()
        if not e:
            e = Event(
                title="Imagine Dragons Live in Moscow",
                event_date=datetime(2026, 7, 1, 19, 0, tzinfo=timezone.utc),
                organizer_id="concert-ru",
                external_event_id="cr-imagine-2026",
                city="Moscow",
            )
            db.session.add(e)
            db.session.commit()
...
def seller_headers(client):
    creds = {
        "email": "seller_orders@example.com",
...
def buyer_headers(client):
    creds = {
        "email": "buyer_orders@example.com",
        "password": "password123",
        "full_name": "Buyer",
    }
    resp = client.post("/auth/register", json=creds)
...
def test_create_order_listing_not_found(client, buyer_headers):
    resp = client.post(
        "/orders", json={"listing_id": str(uuid.uuid4())}, headers=buyer_headers
    )
...
def test_create_order_success(client, buyer_headers, active_listing):
    resp = client.post(
        "/orders", json={"listing_id": active_listing["id"]}, headers=buyer_headers
    )
...
def test_seller_cannot_buy_own_listing(client, seller_headers, active_listing):
    ...
    resp = client.post(
        "/orders", json={"listing_id": active_listing["id"]}, headers=seller_headers
    )
...
def test_full_purchase_flow(client, buyer_headers, active_listing, app):
    """Пополняем кошелёк → создаём заказ → платим → Order=PAID → Reissue=SUCCESS."""
    client.post("/wallet/topup", json={"amount": 5000}, headers=buyer_headers)
    ...
    pay_resp = client.post(
        "/payments/create", json={"order_id": order_id}, headers=buyer_headers
    )
    assert pay_resp.status_code == 201
    assert pay_resp.json["payment"]["status"] == "CONFIRMED"
    assert pay_resp.json["payment"]["provider"] == "wallet"
...
def test_payment_wrong_user(client, seller_headers, buyer_headers, active_listing):
    ...
    order_id = order_resp.json["id"]
    resp = client.post(
        "/payments/create", json={"order_id": order_id}, headers=seller_headers
    )
    assert resp.status_code == 403


def test_payment_already_paid(client, buyer_headers, active_listing):
    """Повторная оплата уже оплаченного заказа возвращает тот же платёж."""
    client.post("/wallet/topup", json={"amount": 5000}, headers=buyer_headers)
    order_resp = client.post(
        "/orders", json={"listing_id": active_listing["id"]}, headers=buyer_headers
    )
...
def test_seller_balance_credited_after_purchase(
    client, seller_headers, buyer_headers, active_listing, app
):
    ...
    client.post(
        "/payments/create",
        json={"order_id": order_resp.json["id"]},
