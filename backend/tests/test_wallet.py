...
def event(app):
    with app.app_context():
        e = Event.query.filter_by(
            organizer_id="qtickets", external_event_id="qt-cska-2026"
        ).first()
        if not e:
            e = Event(
                ...
                external_event_id="qt-cska-2026",
                city="Moscow",
            )
            db.session.add(e)
            db.session.commit()
...
def seller_headers(client):
    ...
    creds = {
        "email": f"w_seller_{uid}@example.com",
        "password": "password123",
        "full_name": "Wallet Seller",
    }
...
def buyer_headers(client):
    uid = uuid.uuid4().hex[:8]
    creds = {
        "email": f"w_buyer_{uid}@example.com",
        "password": "password123",
...
def _sell_and_buy(client, seller_headers, buyer_headers, event, app, price=2000):
    ...
    client.post("/wallet/topup", json={"amount": price + 1000}, headers=buyer_headers)
    order_resp = client.post(
        "/orders", json={"listing_id": listing_id}, headers=buyer_headers
    )
    client.post(
        "/payments/create",
...
def test_topup_increases_balance(client, buyer_headers):
    resp = client.post("/wallet/topup", json={"amount": 1500}, headers=buyer_headers)
    assert resp.status_code == 200
    assert resp.json["balance"] == pytest.approx(1500.0, abs=0.01)


def test_balance_increases_after_sale(
    client, seller_headers, buyer_headers, event, app
):
...
def test_withdraw_insufficient_balance(client, seller_headers):
    resp = client.post(
        "/wallet/withdraw", json={"amount": 999999}, headers=seller_headers
    )
...
def test_withdraw_negative_amount(client, seller_headers):
    resp = client.post(
        "/wallet/withdraw", json={"amount": -100}, headers=seller_headers
    )
    assert resp.status_code == 400
...
def test_withdraw_reduces_balance(client, seller_headers, buyer_headers, event, app):
    ...
    client.post(
        "/wallet/withdraw", json={"amount": withdraw_amount}, headers=seller_headers
    )
